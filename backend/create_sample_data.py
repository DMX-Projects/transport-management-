...
print(f"Branch: {'Created' if created else 'Already exists'} - {branch.name}")
...
party1, created = Party.objects.get_or_create(
    ...
    defaults={
        ...
        'pan': 'BBBPL5678D',
        'address': '789 Customer Street, Connaught Place',
        'city': 'Delhi',
        'state': 'Delhi',
        'pincode': '110001',
        'phone': '9876543212',
...
print(f"Party 1: {'Created' if created else 'Already exists'} - {party1.name}")
...
party2, created = Party.objects.get_or_create(
    ...
    defaults={
        ...
        'address': '321 Transport Avenue, Kothrud',
        'city': 'Pune',
        'state': 'Maharashtra',
        'pincode': '411001',
        'phone': '9876543213',
...
print(f"Party 2: {'Created' if created else 'Already exists'} - {party2.name}")
...
print(f"Truck 1: {'Created' if created else 'Already exists'} - {truck1.truck_number}")
...
print(f"Truck 2: {'Created' if created else 'Already exists'} - {truck2.truck_number}")
...
print(f"Truck 3: {'Created' if created else 'Already exists'} - {truck3.truck_number}")

print("\n✅ Sample data setup complete!")
print(f"Total: {Company.objects.filter(is_deleted=False).count()} companies, {Branch.objects.filter(is_deleted=False).count()} branches, {Party.objects.filter(is_deleted=False).count()} parties, {Truck.objects.filter(is_deleted=False).count()} trucks")
